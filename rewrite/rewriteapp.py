...
class GanRewriteApp(labwidget.Widget):
    ...
    def __init__(self, gw, mask_dir=None, size=256, num_canvases=9):
        ...
        self.request = {}
        self.imgnum_textbox = labwidget.Textbox('0-%d' % (num_canvases - 1), size=4
        ...
        self.rank_textbox = labwidget.Textbox(
            '1', desc='rank: ', size=1, style=inline)
        self.paste_niter_textbox = labwidget.Textbox(
            '2001', desc='paste niter: ', size=2)
        self.paste_piter_textbox = labwidget.Textbox(
            '10', desc='proj every: ', size=1)
        self.paste_lr_textbox = labwidget.Textbox(
            '0.05', desc='paste lr: ', size=2)
        self.erase_btn = labwidget.Button('Erase', style=inline).on('click', self.exec_erase)
    ...
    def widget_html(self):
        ...
        >Rewriting a Deep Generative Model Edited</span>
        ...
        <!--right-->
        <div style="height:{(self.size + 2) * 2 + 50}px; width:{(self.size + 2) * 4 + 22 + 32}px;
          display: inline-block;
          vertical-align: top;
        ...
        {h(self.query_btn)}
        {h(self.rank_textbox)}
        {h(self.paste_niter_textbox)}
        {h(self.paste_piter_textbox)}
